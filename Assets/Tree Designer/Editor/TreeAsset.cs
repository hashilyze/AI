...
namespace TreeDesigner {
    ...
    public class TreeAsset : ScriptableObject {
        ...
        public void AddNode(Object data, Vector2 position, int parentsID) {
            Node newNode = TreeUtility.CreateEmptyNode();
            ...
            newNode.ID = id;
            newNode.ParentsID = parentsID;
            m_nodeTable[parentsID].ChildrenIDs.Add(id);
            m_nodeTable.Add(newNode);
        }
        ...
        public void SortChildren(int id) {
            ...
            m_nodeTable[id].ChildrenIDs.Sort(RectComparer);
        }
        private int RectComparer(int lhs, int rhs) => (int)(m_nodeTable[lhs].Position.x - m_nodeTable[rhs].Position.x);

        [SerializeField] private List<Node> m_nodeTable = new List<Node>() { TreeUtility.CreateEntryNode() };
    }
}